...
def _get_inference_fn(model_fns, features):
    def inference_fn(inputs, state):
        local_features = {
            ...
            "src_image": features["src_image"],
            "ctx_src_image": features["ctx_src_image"],
            "src_ctx_image": features["src_ctx_image"],
            "tgt_ctx_image": features["tgt_ctx_image"],
            # [bos_id, ...] => [..., 0]
...
def create_inference_graph(models, features, params):
    ...
    features["context_source_length"] = tf.tile(features["context_source_length"],
                                        [1, beam_size])
    shape = tf.shape(features["context_source_length"])

    max_length = features["context_source_length"] + decode_length

    # [batch, beam_size, length] => [batch * beam_size, length]
    features["context_source_length"] = tf.reshape(features["context_source_length"],
                                           [shape[0] * shape[1]])
