...
def _evaluate(eval_fn, input_fn, decode_fn, path, config):
    ...
    with graph.as_default():
        ...
        placeholders = {
            ...
            "src_image": tf.placeholder(tf.int32, [None, None], "src_image"),
            "ctx_src_image": tf.placeholder(tf.int32, [None, None], "ctx_src_image"),
            "src_ctx_image": tf.placeholder(tf.int32, [None, None], "src_ctx_image"),
            "tgt_ctx_image": tf.placeholder(tf.int32, [None, None], "tgt_ctx_image")
        }
        ...
        with tf.train.MonitoredSession(session_creator=sess_creator) as sess:
            while not sess.should_stop():
                ...
                outputs = sess.run(predictions, feed_dict={
                    ...
                    placeholders["src_image"]: feats["src_image"],
                    placeholders["ctx_src_image"]: feats["ctx_src_image"],
                    placeholders["src_ctx_image"]: feats["src_ctx_image"],
                    placeholders["tgt_ctx_image"]: feats["tgt_ctx_image"]
                })
                # shape: [batch, len]
