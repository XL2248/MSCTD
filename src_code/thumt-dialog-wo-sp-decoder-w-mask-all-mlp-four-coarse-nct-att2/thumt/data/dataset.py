...
#        ctx_lan_tgt_dataset = tf.data.TextLineDataset(params.language_tgt_context)
        ...
        src_img_idx, ctx_src_img_idx, src_ctx_img_idx, tgt_ctx_img_idx = get_turn_index(filenames[0], params.context_source, params.dialog_src_context, params.dialog_tgt_context, filenames[2])
        src_img_dataset = tf.data.TextLineDataset(src_img_idx)
        ctx_src_img_dataset = tf.data.TextLineDataset(ctx_src_img_idx)
        src_ctx_img_dataset = tf.data.TextLineDataset(src_ctx_img_idx)
        tgt_ctx_img_dataset = tf.data.TextLineDataset(tgt_ctx_img_idx)
...
#        dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset, emo_dataset, ctx_dia_src_dataset, ctx_dia_tgt_dataset, ctx_sty_src_dataset, ctx_sty_tgt_dataset, ctx_lan_src_dataset, ctx_lan_tgt_dataset, position_src_dia_dataset, position_tgt_dia_dataset, position_src_sty_dataset, position_tgt_sty_dataset, position_src_lan_dataset, position_tgt_lan_dataset, sample_dataset))
        dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset, context_source_dataset, position_ctx_src_dataset, ctx_dia_src_dataset, ctx_dia_tgt_dataset, position_src_dia_dataset, position_tgt_dia_dataset, sample_dataset, src_img_dataset, ctx_src_img_dataset, src_ctx_img_dataset, tgt_ctx_img_dataset))
        ...
        # Split string
        dataset = dataset.map(
            lambda src, tgt, ctx_src, pos_ctx_src, ctx_dia_src, ctx_dia_tgt, pos_dia_src, pos_dia_tgt, sample, src_img, ctx_src_img, src_ctx_img, tgt_ctx_img: (
                ...
                tf.string_split([src_img]).values,
                tf.string_split([ctx_src_img]).values,
                tf.string_split([src_ctx_img]).values,
                tf.string_split([tgt_ctx_img]).values
            ),
        ...
        dataset = dataset.map(
            lambda src, tgt, ctx_src, pos_ctx_src, ctx_dia_src, ctx_dia_tgt, pos_dia_src, pos_dia_tgt, sample, src_img, ctx_src_img, src_ctx_img, tgt_ctx_img: {
                ...
                "sample": sample,
                "src_image": src_img,
                "ctx_src_image": ctx_src_img,
                "src_ctx_image": src_ctx_img,
                "tgt_ctx_image": tgt_ctx_img,
...
#        features["emotion"] = emo_table.lookup(features["emotion"])
        ...
        features["src_image"] = index_table.lookup(features["src_image"])
        features["src_ctx_image"] = index_table.lookup(features["src_ctx_image"])
        features["ctx_src_image"] = index_table.lookup(features["ctx_src_image"])
        features["tgt_ctx_image"] = index_table.lookup(features["tgt_ctx_image"])
        ...
        features["src_image"] = tf.to_int32(features["src_image"])
        features["src_ctx_image"] = tf.to_int32(features["src_ctx_image"])
        features["ctx_src_image"] = tf.to_int32(features["ctx_src_image"])
        features["tgt_ctx_image"] = tf.to_int32(features["tgt_ctx_image"])
...
#    return inputs,ctx1,ctx2,ctx3,ctx4,ctx5,ctx6,ctx7,ctx8,ctx9,ctx10,ctx11,ctx12,ctx13
    ...
    sorted_inputs = []
    sorted_ctxs = []
    dialog_src_context, pos_src_dia, dialog_tgt_context, pos_tgt_dia, style_src_context, pos_src_sty, img1, img2, img3, img4 = [], [], [], [],[], [], [], [],[], []
...
#        sorted_ctxs.append(ctxs[index])
        ...
        img1.append(ctx7[index])
        img2.append(ctx8[index])
        img3.append(ctx9[index])
        img4.append(ctx10[index])

        sorted_keys[index] = i

    return sorted_keys, sorted_inputs, dialog_src_context, pos_src_dia, dialog_tgt_context, pos_tgt_dia, style_src_context, pos_src_sty, img1, img2, img3, img4
...
#        code.interact(local=locals())
        dataset = dataset.padded_batch(
            params.eval_batch_size,
            {
                ...
                "sample_length": [],
                "src_image": [tf.Dimension(None)],
                "ctx_src_image": [tf.Dimension(None)],
                "src_ctx_image": [tf.Dimension(None)],
                "tgt_ctx_image": [tf.Dimension(None)],
            ...
            },
            {
                ...
                "sample": params.pad,
                "sample_length": 0,
                "src_image": params.pad,
                "ctx_src_image": params.pad,
                "src_ctx_image": params.pad,
                "tgt_ctx_image": params.pad,
...
def get_inference_input(inputs, data_type, params):
    ...
    with tf.device("/cpu:0"):
        ...
        if data_type == "img":
            features["source"] = index_table.lookup(features["source"])
        return features
