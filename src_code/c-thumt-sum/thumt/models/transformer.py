...
#        img_emb = tf.get_variable(name='embedding_weights', shape=(142871, 1000), initializer=weights_initializer, trainable=False)
    ...
    img_input = tf.layers.dense(img_input, hidden_size, activation=tf.nn.tanh)
...
#    encoder_input = tf.layers.dense(multimodal, hidden_size, activation=tf.nn.tanh)
    ...
    ##### sum
    multimodal = tf.concat([encoder_output, image_input], -1)

    encoder_output = tf.layers.dense(multimodal, hidden_size, activation=tf.nn.tanh)
