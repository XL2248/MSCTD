...
def get_training_input(filenames, params):
    ...
    with tf.device("/cpu:0"):
        ...
        img_dataset = tf.data.TextLineDataset(filenames[2])

        dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset, img_dataset))
        ...
        features["image"] = tf.to_int32(features["image"])
...
def sort_input_file_img(filename, f1, reverse=True):
    # Read file
    with tf.gfile.Open(filename) as fd:
        inputs = [line.strip() for line in fd]
    ...
    for i, (index, _) in enumerate(sorted_input_lens):
        sorted_inputs.append(inputs[index])
        sorted_keys[index] = i
        sorted_imgs.append(ctx1[index])

    return sorted_keys, sorted_inputs, sorted_imgs
...
def get_inference_input(inputs, data_type, params):
    ...
    with tf.device("/cpu:0"):
        ...
        if data_type == "img":
            ...
            # Convert tuple to dictionary
            dataset = dataset.map(
                lambda x: {"source": x[0], "source_length": tf.shape(x)[0]},
                num_parallel_calls=params.num_threads
            )
            dataset = dataset.padded_batch(
