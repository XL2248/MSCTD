...
def parse_args():
    ...
    parser.add_argument("--valid_object", type=str,
                        help="Path of validation file")
    parser.add_argument("--valid_object_mask", type=str,
                        help="Path of validation file")
...
#            position_file_ctx_src, mask_file = get_turn_position_eos(params.dev_context_source)
        ...
        sorted_keys, sorted_inputs, dialog_src_context, pos_src_dia, dialog_tgt_context, pos_tgt_dia, dialog_context_source, pos_ctx_src, valid_index = dataset.sort_input_file_ctx(args.input, params.dev_dialog_src_context, position_file_src_dia, params.dev_dialog_tgt_context, position_file_tgt_dia, params.dev_context_source, position_file_ctx_src, params.valid_index)
        # Build input queue
        img_mask_dataset = np.memmap(args.valid_object_mask, mode="r", dtype=np.bool, shape=(len(valid_index), 2))
        img_dataset = np.memmap(args.valid_object, mode="r", dtype=np.float32, shape=(len(valid_index), 2, 2048))
        img, img_mask = [], []
#        code.interact(local=locals())
        for idx in valid_index:
            img.append(img_dataset[int(idx)])
            img_mask.append(img_mask_dataset[int(idx)])

        dev_obj_mask = np.array(img_mask)
        dev_obj = np.array(img)
...
#        features_ctx = dataset.get_inference_input(sorted_ctxs, params)
        ...
        obj_features = dataset.get_inference_input(dev_obj, "img", params)
        obj_mask_features = dataset.get_inference_input(dev_obj_mask, "img_mask", params)
        features["image"] = obj_features["source"]
        features["image_mask"] = obj_mask_features["source"]
        # Create placeholders
        placeholders = []
        ...
        for i in range(len(params.device_list)):
            placeholders.append({
                ...
                "sample_length": tf.placeholder(tf.int32, [None],
                                                "sample_length_%d" % i),
                "image": tf.placeholder(tf.float32, [None, 2, 2048], "image_%d"%i),
                "image_mask": tf.placeholder(tf.float32, [None, 2], "image_mask_%d"%i),
