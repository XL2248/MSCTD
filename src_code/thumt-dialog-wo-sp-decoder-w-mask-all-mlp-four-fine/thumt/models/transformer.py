...
def transformer_encoder(inputs, bias, params, image, dia_mask=None, dtype=None, scope=None, trainable=True, get_first_layer=False):
    with tf.variable_scope("encoder", dtype=dtype,
        ...
        for layer in range(params.num_encoder_layers):
            if image is not None and layer > 0:
...
#    obj_mask = tf.expand_dims(obj_mask_input, -1)
    dec_ctx_src_mask = tf.concat([ctx_src_mask, obj_mask_input], -1)
#    print("obj_mask_input",ctx_src_mask, obj_mask_input)
    inv_obj_mask_input = tf.zeros_like(obj_mask_input,dtype=tf.float32)
    tmp_ctx_mask = tf.concat([ctx_mask, inv_obj_mask_input], -1)
    dec_top_mask = tf.equal(tmp_ctx_mask, dec_ctx_src_mask)
    dec_top_mask = tf.cast(dec_top_mask, tf.float32)

    top_mask = tf.equal(ctx_mask, ctx_src_mask)
    top_mask = tf.cast(top_mask, tf.float32)
    print("obj_mask_input",ctx_src_mask, obj_mask_input, dec_top_mask, top_mask)
    inf = -1e9
    if dtype is None:
        dtype = tf.float32
    if dtype != tf.float32:
...
#            emo_emb = tf.Variable(_load_embedding(params.vocabulary["emotion"], params), name="emo_embedding", trainable=False)
    ...
    '''
#    code.interact(local=locals())
    com_mask = tf.concat([ctx_src_mask, obj_mask_input], -1)
    ...
    encoder_output, first_layer_output = transformer_encoder(encoder_input, enc_attn_bias, params, None)
...
#        context_lan_src = transformer_context(context_input, ctx_attn_bias, params)
    context_source, _ = transformer_encoder(context_input, ctx_attn_bias, params, obj_input, dia_mask=dia_mask)
...
#        print("dia_mask")
        context_dia_src, _ = transformer_encoder(context_input, ctx_attn_bias, params, image, get_first_layer)
...
#        image = None
        context_dia_tgt, _ = transformer_encoder(context_input, ctx_attn_bias, params, image, get_first_layer)
        ...
        sa_tgt, _ = transformer_encoder(sa_input, sa_attn_bias, params, image, get_first_layer)
...
#        context_dia_tgt = transformer_context(context_input, ctx_attn_bias, params)
    ...
    if mode == "infer":
        dia_mask = dec_top_mask
        return encoder_output, context_dia_src, context_dia_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sa_tgt, dia_mask, com_mask

    return encoder_output, context_dia_src, context_dia_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sa_tgt, dec_top_mask, com_mask
...
def decoding_graph(features, state, mode, params):
    ...
    if params.use_coherence or params.use_cluts or params.use_clus:
        ...
        image = None
        tgt_encoder_output, _ = transformer_encoder(targets_, tgt_attn_bias, params, image)
...
#    turn_lan_tgt_pos_emb = state["position_lan_tgt"]
    ...
    img_enc_attn_bias = layers.attention.attention_bias(com_mask, "masking",
                                                    dtype=dtype)
    inf = -1e9
    if dtype is None:
        dtype = tf.float32
...
#    code.interact(local=locals())
    ...
    if mode != "infer":
        ...
        decoder_output = transformer_decoder(decoder_input, context_source_output,
                                             dec_attn_bias, img_enc_attn_bias,
...
#                                             params)
        ...
        if params.use_mrg: # monolingual response generation task
            mrg_decoder_output = transformer_decoder(decoder_input, context_dia_tgt_output,
                                                 dec_attn_bias, ctx_dia_tgt_attn_bias,
                                                 params)
        if params.use_crg: # monolingual response generation task
            crg_decoder_output = transformer_decoder(decoder_input, context_dia_src_output,
...
#        code.interact(local=locals())
        ...
        part_mask = tf.cast(ori_mask, dtype)
        dia_mask = inf * part_mask
        decoder_outputs = transformer_decoder(decoder_input, context_source_output,
                                              dec_attn_bias, img_enc_attn_bias,
...
def model_graph(features, mode, params):
    encoder_output, context_dia_src, context_dia_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sample_output, dia_mask, com_mask = encoding_graph(features, mode, params)
    state = {
...
class Transformer(NMTModel):
    ...
    def get_inference_func(self):
        def encoding_fn(features, params=None):
            ...
            with tf.variable_scope(self._scope):
                encoder_output, context_dia_src, context_dia_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sample_output, dia_mask, com_mask = encoding_graph(features, "infer", params)
