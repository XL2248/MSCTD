...
def _load_mask_embedding(mode, params, uniform_scale = 0.25, dimension_size = 300, embed_file='glove'):
    n = NUMBER
    path = "/path/to/multimodal_mt/object_features/all.objects_mask.mmap"
...
def encoding_graph(features, mode, params):
    ...
    print("image_input", obj_input, obj_input)
    ...
    encoder_input = tf.concat([encoder_input, obj_input], -2)
    encoder_input += multimodal_pos
...
def decoding_graph(features, state, mode, params):
    ...
    with tf.variable_scope("img_embedding", reuse=tf.AUTO_REUSE):
        mask_initializer = tf.constant_initializer(_load_mask_embedding(mode, params))
        mask_emb = tf.get_variable(name='mask_weights', shape=(NUMBER, MAX_OBJECTS), initializer=mask_initializer, trainable=False)
    obj_mask_input = tf.nn.embedding_lookup(mask_emb, features["image"])
    ...
    if mode != "infer":
        ...
        return log_prob, {"encoder": encoder_output, "decoder": decoder_state, "enc_attn_bias": enc_attn_bias}
...
def model_graph(features, mode, params):
    encoder_output, enc_attn_bias = encoding_graph(features, mode, params)
    state = {
...
class Transformer(NMTModel):
    ...
    def get_inference_func(self):
        def encoding_fn(features, params=None):
            ...
            with tf.variable_scope(self._scope):
                encoder_output, enc_attn_bias = encoding_graph(features, "infer", params)
