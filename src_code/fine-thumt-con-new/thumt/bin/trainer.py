...
def parse_args(args=None):
    ...
    # input files
    parser.add_argument("--input", type=str, nargs=5,
...
def override_parameters(params, args):
    ...
    params.vocabulary = {
        ...
        "index": vocabulary.load_vocabulary(params.vocab[2])
    }
...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        if params.validation and params.references[0]:
            files = [params.validation] + [params.valid_index] + list(params.references)
            eval_inputs = dataset.sort_and_zip_files(files)
            eval_input_fn = dataset.get_evaluation_input_img
        else:
            eval_input_fn = None
