...
def get_training_input(filenames, params):
    ...
    with tf.device("/cpu:0"):
        ...
        print(params.image_num,filenames[4])
        img_mask_dataset = tf.data.Dataset.from_tensor_slices(np.memmap(filenames[4], mode="r", dtype=np.bool, shape=(params.image_num, 2)))
        img_dataset = tf.data.Dataset.from_tensor_slices(np.memmap(filenames[3], mode="r", dtype=np.float32, shape=(params.image_num, 2, 2048)))
