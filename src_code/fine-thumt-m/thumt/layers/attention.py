...
def multihead_attention(queries, memories, bias, num_heads, key_size,
                        value_size, output_size, object_features, keep_prob=None, output=True,
...
#            obj_comb = linear(object_features, size, True, True, scope="obj_qkv_transform")
        ...
        if object_features is not None:
            multimodal = tf.concat([queries, object_features], -2)
            obj_comb = linear(multimodal, size, True, True, scope="multimodal_qkv_transform")

            q, img_k, img_v = tf.split(obj_comb, [key_size, key_size, value_size],
                              axis=-1)
