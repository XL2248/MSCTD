...
def _load_embedding(mode, params, uniform_scale = 0.25, dimension_size = 300, embed_file='glove'):
    n = 0
#    word_vectors = np.memmap("/apdcephfs/share_47076/yunlonliang/multimodal_mt/coarse_features/all.features.mmap", mode="r", dtype=np.float32, shape=(142871, 1000))
    word_vectors = np.memmap("/apdcephfs/share_47076/yunlonliang/multimodal_mt/coarse_features/all.ende.features.mmap", mode="r", dtype=np.float32, shape=(30370, 1000))
...
def transformer_decoder(inputs, memory, image_output, bias, mem_bias, params, state=None,
                        dtype=None, scope=None):
...
def encoding_graph(features, mode, params):
    ...
    length = tf.shape(encoder_input)[-2]
...
#        img_emb = tf.get_variable(name='embedding_weights', shape=(142871, 1000), initializer=weights_initializer, trainable=False)
    ...
    img_input = tf.nn.embedding_lookup(img_emb, features["image"])
    image_input = tf.tile(tf.expand_dims(img_input, -2), [1, length, 1])
    print("image_input", image_input)
    #multimodal = tf.concat([encoder_input, image_input], -1)

    #encoder_input = tf.layers.dense(multimodal, hidden_size, activation=tf.nn.tanh)
    image_input = tf.layers.dense(image_input, hidden_size, activation=tf.nn.tanh)
    ...
    return encoder_output, image_input
...
def decoding_graph(features, state, mode, params):
    ...
    image_output = state["image_output"]

    if mode != "infer":
        decoder_output = transformer_decoder(decoder_input, encoder_output, image_output,
                                             dec_attn_bias, enc_attn_bias,
                                             params)
    else:
        decoder_input = decoder_input[:, -1:, :]
        dec_attn_bias = dec_attn_bias[:, :, -1:, :]
        decoder_outputs = transformer_decoder(decoder_input, encoder_output, image_output,
                                              dec_attn_bias, enc_attn_bias,
        ...
        return log_prob, {"encoder": encoder_output, "decoder": decoder_state, "image_output": image_output}
...
def model_graph(features, mode, params):
    encoder_output, image_output = encoding_graph(features, mode, params)
    state = {
...
class Transformer(NMTModel):
    ...
    def get_inference_func(self):
        def encoding_fn(features, params=None):
            ...
            with tf.variable_scope(self._scope):
                encoder_output, image_output = encoding_graph(features, "infer", params)
