...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        sorted_keys, sorted_inputs, sorted_images = dataset.sort_input_file_img(args.input, args.valid_index)

        # Build input queue
        features = dataset.get_inference_input(sorted_inputs, "x", params)
        features_img = dataset.get_inference_input(sorted_images, "img",  params)
        features["image"] = features_img["source"]
        # Create placeholders
        placeholders = []
        ...
        for i in range(len(params.device_list)):
            placeholders.append({
                ...
                "source_length": tf.placeholder(tf.int32, [None],
                                                "source_length_%d" % i),
                "image": tf.placeholder(tf.int32, [None],
                                         "image_%d" % i)
            })
