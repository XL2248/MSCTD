...
model_name=pretrain_zh2en_fine_TCT_uc1_crg${crg}_mrg${mrg}_coh${coh}
python $work_dir/$code_dir/thumt/bin/trainer.py \
  ...
  --parameters=device_list=[0,1,2,3],update_cycle=1,eval_steps=1000,train_steps=$train_steps,batch_size=4096,max_length=128,constant_batch_size=False,residual_dropout=0.4,attention_dropout=0.1,relu_dropout=0.1,hidden_size=512,filter_size=2048,num_heads=8,shared_source_target_embedding=True,kl_annealing_steps=$kl_steps1,kl_annealing_steps2=$kl_steps2,use_crg=$crg,use_mrg=$mrg,use_coherence=$coh,use_clus=$clus,use_cluts=$cluts,use_clm=$clm,start_steps=$start_steps,position_info_type="absolute",num_encoder_layers=$num,num_decoder_layers=$num,image_num=$image_num,dev_object=$image_data/dev.objects.mmap,dev_object_mask=$image_data/dev.objects_mask.mmap
