...
uc=1
image_num=62593
model_name=zh2en_model_fine_m_4gpu_${steps}0w_sent_layer${layer}_uc${uc}
#model_name=zh2en_model_fine_ours_test_debug1
python $work_dir/$code_dir/thumt/bin/trainer.py \
  --model transformer \
  --output $work_dir/models_zhen/$model_name \
  --input $train_data/train_zh.txt $train_data/train_en.txt $train_data/train_image_idx.txt $image_data/train.objects.mmap $image_data/train.objects_mask.mmap \
  ...
  --parameters=device_list=[0,1,2,3],update_cycle=$uc,eval_steps=2000,train_steps=${steps}00000,batch_size=4096,max_length=128,constant_batch_size=False,residual_dropout=0.2,attention_dropout=0.1,relu_dropout=0.1,hidden_size=512,filter_size=2048,num_heads=8,shared_source_target_embedding=True,position_info_type="absolute",num_encoder_layers=$layer,num_decoder_layers=$layer,image_num=$image_num,dev_object=$image_data/dev.objects.mmap,dev_object_mask=$image_data/dev.objects_mask.mmap
